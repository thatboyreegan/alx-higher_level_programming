#!/usr/bin/python3
"""Defines a function that multiplies two matrices"""


def matrix_mul(m_a, m_b):
    """
    Function multiplies two matrices

    Args:
        m_a( list, int/float): first list
        m_b( list, int/float): second list

    Raises:
        TypeError: when m_a or m_b is not a list of lists of ints/floats.
        TypeError: when m_a or m_b is empty.
        TypeError: when m_a or m_b has different-sized rows.
        ValueError: when m_a and m_b cannot be multiplied.

    Returns:
        a new matrix with the result of the multiplication
    """
    if type(m_a) is not list:
        raise TypeError("m_a must be a list")
    if type(m_b) is not list:
        raise TypeError("m_b must be a list")
    if not all(isinstance(row, list) for row in m_a):
        raise TypeError("m_a must be a list of lists")
    if not all(isinstance(row, list) for row in m_b):
        raise TypeError("m_b must be a list of lists")
    if m_a == [] or m_a == [[]]:
        raise TypeError("m_a can't be empty")
    if m_b == [] or m_b == [[]]:
        raise TypeError("m_b can't be empty")
    if not all((isinstance(elem, int) or isinstance(elem, float))
               for elem in [num for row in m_a for num in row]):
        raise TypeError("m_a should contain only integers or floats")
    if not all((isinstance(elem, int) or isinstance(elem, float))
               for elem in [num for row in m_b for num in row]):
        raise TypeError("m_b should contain only integers or floats")
    if not all(len(row) == len(m_a[0]) for row in m_a):
        raise TypeError("each row of m_a must should be of the same size")
    if not all(len(row) == len(m_b[0]) for row in m_b):
        raise TypeError("each row of m_b must should be of the same size")
    if len(m_a[0]) != len(m_b):
        raise ValueError("m_a and m_b can't be multiplied")

    temp_l = []
    for r in range(len(m_b[0])):
        new_row = []
        for c in range(len(m_b)):
            new_row.append(m_b[c][r])
        temp_l.append(new_row)

    n_matrix = []
    for row in m_a:
        new_row = []
        for col in temp_l:
            result = 0
            for i in range(len(temp_l[0])):
                result += row[i] * col[i]
            new_row.append(result)
        n_matrix.append(new_row)

    return n_matrix
